...
export default async function auth(req, res) {
  ...
  try {
    ...
    const userOrganizationIds =
      userOrganizations.data?.map((org) => org.organization_id) || [];
    const userProjectIds =
      userProjects.data?.map((proj) => proj.project_id) || [];
    ...
    if (documentData) {
      ...
      highestAccess = compareAccessLevels(
        compareAccessLevels(
          documentAccess,
          hasProjectAccess ? "read" : undefined
        ),
    ...
    if (highestAccess === "write" || highestAccess === "admin") {
      ...
      liveblocksSession.allow(document_id, [
        "room:read",
        "room:presence:write",
        "comments:write",
      ]);
